...
@router.post("/user/login", response_model=backend.mjc.model.schema.user.Token)
...
@router.post("/user/register", response_model=backend.mjc.model.schema.user.Profile)
async def register(db: SessionDep, user: backend.mjc.model.schema.user.UserCreate):
    return user_service.register(db, user)


@router.get("/user/{username}", response_model=backend.mjc.model.schema.user.Profile)
